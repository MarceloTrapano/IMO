...
func main() {
	var (
		...
		parameters             []int          // parametry metod
	)
	args := os.Args[1:]
	if len(args) < 2 {
		fmt.Println("usage: go run main.go <path_to_instance> method [time limit (ms)] [greedy heuristic] [local search algorithm]")
		return
	}
	if len(args) > 2 {
	...
	if len(args) > 3 {
		heuristic_algorithm = args[3]
	}
	if len(args) > 4 {
		local_search_algorithm = args[4]
	}
	if len(args) > 5 {
		for i := 5; i < len(args); i++ {
			param, err := strconv.Atoi(args[i])
			if err != nil {
				fmt.Println("Error parsing parameter - not int:", args[i])
				return
			}
	...
	for i := 0; i < num_of_rep; i++ {
		fmt.Printf("Trial: %d\n", i+1)
		start_time = time.Now()
		order, iter, err = solver.CustomMethod(nodes, distance_matrix, time_limit, heuristic_algorithm, local_search_algorithm, method, parameters...)
	...
	os.WriteFile("Res_GC_CustomHAEMut_KroB200.json", finalJson, 0644)
}
